<?php
	...
	else if(isset($_POST['update']))
	{
		...
		if($_FILES["imagembanner"]["tmp_name"] != "")
		{
			$_UP['pasta'] = '../../assets/img/banners/';
			...
			if (move_uploaded_file($_FILES['imagembanner']['tmp_name'], $_UP['pasta'] . $nome_final)) {
				...
				// Upload efetuado com sucesso, exibe uma mensagem e um link para o arquivo
			  	if(bannerrandomico_update($p))
				{
					header("location: ../../index.php?m=O banner foi alterado com sucesso!");
				}
				else
				{
					
					header('location: ../../index.php?m=O banner foi alterado com sucesso, mas não foi possível alterar seus dados!');
				}
			  
			} else {
				
			  // Não foi possível fazer o upload, provavelmente a pasta está incorreta
			  header("location: ../../index.php?m=Não foi possível alterar o banner, tente novamente.");
			}
		}
		else
		{
